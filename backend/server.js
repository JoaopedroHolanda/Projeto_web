...
app.get('/minhas-inscricoes', verificarToken, (req, res) => {
    const usuarioId = req.userId
    const query = `
        SELECT e.id as evento_id, e.titulo as evento, u.nome, u.email, i.status
        FROM inscricoes i
        JOIN eventos e ON i.evento_id = e.id
        JOIN usuarios u ON i.usuario_id = u.id  -- Change to i.usuario_id to get the name of the user who registered
        WHERE i.usuario_id = ?
    `
    db.query(query, [usuarioId], (err, results) => {
        if (err) return res.status(500).json({ erro: 'Erro ao buscar inscrições do usuário' })
        res.json(results)
    })
})


app.post('/remover-inscricao', verificarToken, (req, res) => {
    ...
    db.query(query, [evento_id, usuario_id], (err, result) => {
        if (err) return res.status(500).json({ erro: 'Erro ao remover inscrição' })
...
app.get('/certificados/eventos/:id/participantes', verificarToken, (req, res) => {
    ...
    db.query('SELECT * FROM eventos WHERE id = ? AND usuario_id = ?', [id, usuarioId], (err, results) => {
        ...
        const query = `
            ...
            FROM inscricoes i
            JOIN usuarios u ON i.usuario_id = u.id
            WHERE i.evento_id = ? AND i.status = 'Participou'
        `
...
app.post('/inscricoes/atualizar', verificarToken, (req, res) => {
    ...
    const promises = inscricoes.map(inscricao => {
        return new Promise((resolve, reject) => {
            db.query('SELECT * FROM eventos WHERE id = ? AND usuario_id = ?', [inscricao.evento_id, usuarioId], (err, results) => {
                if (err) return reject(err)
                if (results.length === 0) return reject('Você não tem permissão para gerenciar as inscrições deste evento')
...
app.get('/inscricoes/:eventoId', verificarToken, (req, res) => {
    ...
    db.query('SELECT * FROM eventos WHERE id = ? AND usuario_id = ?', [eventoId, usuarioId], (err, results) => {
        if (err) return res.status(500).json({ erro: 'Erro ao verificar permissão' })
        if (results.length === 0) return res.status(403).json({ erro: 'Você não tem permissão para acessar as inscrições deste evento' })
